...
const ExercisePage = () => {
  ...
  const handleApprovalStatusChange = async (
    ...
    try {
      setRows((prevRows) =>
        prevRows.map((row) =>
          row.id === rowData.id ? { ...row, approved: newApprovalStatus } : row
        )
      );
        ...
        header: () => (
          <div className="w-10 h-10 flex items-center justify-center">
            <input
              ...
              defaultChecked={selectedRows.length > 0 ? true : false}
            />
          </div>
        ),
        cell: (info) => (
  ...
  return (
    <main className="flex flex-col bg-transparent text-black h-[calc(100vh-88px)] overflow-hidden">
      <div className="w-full flex flex-col gap-4 h-full">
        <div className="w-full flex flex-row items-center justify-between py-2 px-6 border-b-[1px] border-gray-200">
