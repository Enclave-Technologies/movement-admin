...
const GoalList = ({ goals, setGoals, clientData, pageLoading }) => {
  ...
  const updateGoalInBackend = useCallback(
    async (id: string, completed: boolean) => {
      try {
        ...
        console.log(
          `Updating goal ${id} to ${completed ? "completed" : "not completed"}`
        );
      } catch (error) {
  ...
  const deleteGoal = async (goalId, goalType) => {
    if (window.confirm("Are you sure you want to delete this goal?")) {
      try {
        await axios.delete(`${API_BASE_URL}/mvmt/v1/client/goals/${goalId}`, {
          withCredentials: true,
        });
      ...
      if (goals?.length > 0) {
        updatedGoals = updatedGoals.map((updatedGoal) => {
          const existingGoal = goals.find(
            (g) => g.type.toLowerCase() === updatedGoal.type.toLowerCase()
          );
          if (existingGoal) {
            return {
              ...existingGoal,
              goals:
                existingGoal.type.toLowerCase() === `${goalType}`.toLowerCase()
                  ? [
                      ...existingGoal.goals,
                      {
                        id: data.id,
        ...
        updatedGoals = updatedGoals.map((updatedGoal) =>
          updatedGoal.type.toLowerCase() === `${goalType}`.toLowerCase()
            ? {
                ...updatedGoal,
                goals: [
                  {
        ...
        category.goals.forEach((goal) => {
          if (goal.id === editedGoal.id) {
            ...
            const targetCategory = updatedGoals.find(
              (cat) => cat.type.toLowerCase() === `${editedGoal.type}`
            );
            if (targetCategory) {
            ...
            const targetCategory = updatedGoals.find(
              (cat) => cat.type.toLowerCase() === category.type.toLowerCase()
            );
            if (targetCategory) {
  ...
  return (
    ...
    `}
      ...
      <div className="flex flex-col w-full gap-8">
        {goals?.map((goal) => (
          <div key={goal.id} className="flex flex-col w-full items-start gap-4">
            <h1 className="uppercase text-xl font-bold">{goal.type} Goals</h1>
              ...
              {goal.goals.length > 0 ? (
                goal.goals.map((goalItem) => (
                  <GoalTile
                    ...
                    onEdit={() => editGoal(goalItem, goal.type)}
                    onDelete={() => deleteGoal(goalItem.id, goal.type)}
                  />
                ))
              ) : (
                <div className="text-center py-4 px-6 bg-gray-100 rounded-md shadow-sm">
                  ...
                  <p className="text-gray-400 text-xs mt-1 uppercase">
                    Click &ldquo;Add Goal&rdquo; to get started
                  </p>
                </div>
              )}
            </div>
