...
const GoalList = ({
    ...
    if (goalToDelete) {
      try {
        const updatedGoals = goals.map((category) => ({
          ...category,
          goals: category.goals.filter((goal) => goal.id !== goalToDelete),
        }));
  ...
  const addGoal = (goalType, newGoal, notes, deadline) => {
  ...
  return (
    ...
    `}
      ...
      <div className="flex flex-col w-full gap-8">
        {goals?.map((goal, idx) => (
          <div key={idx} className="flex flex-col w-full items-start gap-4">
            <h1 className="capitalize text-xl font-bold">{goal.type} Goals</h1>

            <div className="flex flex-col gap-1 w-full">
              {goal.goals.length > 0 ? (
                goal.goals.map((goalItem) => (
                  <React.Fragment key={goalItem.id}>
                    {goalToDelete === goalItem.id && (
                    ...
                    <GoalTile
                      ...
                      onDelete={() => handleDeleteGoal(goalItem.id)}
                    />
                  </React.Fragment>
                ))
              ) : (
                <div className="text-center py-4 text-gray-500">
                  No {goal.type} goals yet.
                </div>
              )}
            </div>
          </div>
