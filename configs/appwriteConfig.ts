...
const createSessionClient = async () => {
    ...
    console.log("[createSessionClient] Attempting to create session client...");
    try {
        ...
        if (!sessionCookie || !sessionCookie.value) {
            ...
            console.log(
                "[createSessionClient] Session cookie found. Attempting to parse and set session."
            );
            ...
            try {
                ...
                if (sessionData && sessionData.session) {
                    const secretToSet = sessionData.session;
                    // WARNING: Logging sensitive session secret for debugging ONLY. Remove this log later.
                    console.log(
                        `[createSessionClient] Attempting to set session with secret: ${secretToSet}`
                    );
                    client.setSession(secretToSet);
                    console.log(
                        "[createSessionClient] Session set successfully from parsed cookie JSON."
                    );
                } else {
                    console.warn(
                ...
                try {
                    const rawSecretToSet = sessionCookie.value;
                    // WARNING: Logging sensitive session secret for debugging ONLY. Remove this log later.
                    console.log(
                        `[createSessionClient] Attempting to set session with raw secret: ${rawSecretToSet}`
                    );
                    client.setSession(rawSecretToSet); // Fallback: try using the raw value
                    console.log(
                        "[createSessionClient] Session set successfully using raw cookie value."
                    );
